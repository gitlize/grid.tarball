...
\para -i    {}                             {}
   ...
   Program bude pou¾ívat pouze intenzivní algoritmus a testy.
...
\para -lf <file>                 implicitní hodnota: "stdout"

   Místo na terminál bude výstup programu smìrován
   do souboru. Pokud tento soubor existuje, bude na 
   zaèátku èinnosti programu vymazán.
...
\bod   "-out 4" ... neukonèená øe¹ení se tisknou v~ka¾dém
                    kroku bez ohledu na nastavení pause.
                    
\para -of <file>                 implicitní hodnota: "stdout"

   Místo na terminál bude tisk øe¹ení smìrován
   do souboru. Pokud tento soubor existuje, bude na 
   zaèátku èinnosti programu vymazán.
...
øe¹ení. V~takovém pøípadì program pøechází do krokù typu {\bf R} a {\bf C},
ve kterém znovu projde øádky a sloupce, tentokrát intenzivním algoritmem.
Tento algoritmus je nepatrnì pomalej¹í, ale nevynechá nic, 
co by mohl odhalit èlovìk zamìøující se izolovanì na øádky nebo sloupce. 
Po nalezení zmìny v~lu¹»ovce v~rámci
krokù typu {\bf R} a {\bf C} se program vrací do re¾imu støídání krokù typu 
{\bf r} a {\bf c} a¾ do chvíle, kdy znovu tímto zpùsobem nelze 
odhalit dal¹í zmìnu.

Mù¾e se stát, ¾e ani po aplikaci krokù typu {\bf R} a {\bf C} není odhalena
...
krokù typu~{\bf r} a~{\bf c} s~obèasným pou¾itím krokù typu 
{\bf R} a {\bf C} a¾ do situace, kdy najde øe¹ení nebo zjistí, ¾e návrh
...
pøechází do krokù typu~{\bf R}, {\bf C} a {\bf E} a pøípadnì také do kroku
typu~{\bf t}.
...
pou¾ívá program "mk" ("http://frix.fri.utc.sk/~johny/mk43frm.php"). 
